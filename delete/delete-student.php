...
if (isset($_POST['deleteStudent'])) {
    if ($_GET['archive'] == 1) {
        if ((new StudentMap())->deleteArchiveStudentById($id)) {
            header('Location: ../archive/archive-student?message=ok');
            exit();
        }
        header('Location: ../archive/archive-student?message=err');
        exit();
    } else {
        (new StudentMap())->deleteStudentById($id);
